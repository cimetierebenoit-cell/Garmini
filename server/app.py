...
# Pushcut : deux modes pour faire LIRE le feedback Gemini dans les AirPods.
#
#   PUSHCUT_MODE = "server"  (défaut)
#     -> POST /execute?shortcut=<nom> sur l'Automation Server, avec le TEXTE
#        COMPLET dans `input`. Le Raccourci fait "Énoncer le texte" (aucune
#        limite de longueur). Nécessite Pushcut au PREMIER PLAN (écran allumé,
#        verrouillage auto désactivé), AirPods reliés à ce téléphone.
#
#   PUSHCUT_MODE = "notifications"
#     -> /notifications/<nom>. Le texte complet est DÉCOUPÉ en plusieurs
#        notifications "urgentes" successives, lues l'une après l'autre par
#        "Annoncer les notifications". Marche écran VERROUILLÉ, dans la poche.
...
def _split_text(text, max_chars):
    """Découpe `text` en morceaux <= max_chars sans couper un mot.

    On essaie de couper à une fin de phrase, sinon à un espace.
    """
    text = (text or "").strip()
    chunks = []
    while len(text) > max_chars:
        window = text[:max_chars]
        # Cherche la dernière ponctuation de fin de phrase dans la fenêtre.
        cut = max(window.rfind(". "), window.rfind("! "), window.rfind("? "))
        if cut < max_chars * 0.5:        # pas de bonne coupe -> on coupe au mot
            cut = window.rfind(" ")
        if cut <= 0:                      # mot unique trop long -> coupe brute
            cut = max_chars
        else:
            cut += 1
...
def send_to_pushcut(text, km_number):
    """Fait lire le feedback Gemini COMPLET dans les AirPods via Pushcut.

    Retourne (ok: bool, info: str).

    Mode "server"        : un seul POST /execute?shortcut=... avec le texte
                           complet dans `input` ; le Raccourci l'énonce
                           (aucune limite). Nécessite l'Automation Server au
                           premier plan.
    Mode "notifications" : découpe le texte en plusieurs notifications urgentes
                           successives, lues par "Annoncer les notifications"
                           écran verrouillé.
    """
    if not text:
        return (False, "empty")
    ...
    try:
        if PUSHCUT_MODE == "server":
            # Texte complet -> Raccourci "Énoncer le texte" sur l'Automation
            # Server. timeout=nowait : on n'attend pas la fin de la lecture.
            url = (
                f"https://api.pushcut.io/{PUSHCUT_SECRET}/execute"
                f"?shortcut={quote(PUSHCUT_SHORTCUT)}&timeout=nowait"
            )
            resp = requests.post(
                url,
                headers={"Content-Type": "application/json"},
            ...
            if 200 <= resp.status_code < 300:
                return (True, f"server HTTP {resp.status_code}")
            return (False, f"server HTTP {resp.status_code} {resp.text[:200]}")

        # Mode notifications : on découpe et on envoie séquentiellement.
        ...
        for i, chunk in enumerate(chunks, start=1):
            ...
            if i < total:
                # On attend que Siri ait fini de lire CE morceau (titre + texte)
                # avant d'envoyer le suivant. Sinon iOS regroupe les notifs et
                # n'énonce que la dernière. Délai = durée de lecture estimée + marge.
                read_time = (len(title) + len(chunk)) / max(PUSHCUT_TTS_CPS, 1.0)
                delay = read_time + PUSHCUT_NOTIFY_BUFFER
                log.info("PUSHCUT km %s : notif %s/%s envoyée, attente %.0fs",
                         km_number, i, total, delay)
                time.sleep(delay)
        return (True, f"notifications {total} envoyée(s)")
