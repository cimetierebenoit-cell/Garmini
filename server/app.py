...
# Pushcut : automation déclenchée avec le feedback Gemini en entrée.
PUSHCUT_URL = os.environ.get(
    "PUSHCUT_URL",
    "https://api.pushcut.io/zK_G6KUQHQcPfHN-g8Dyi/execute?shortcut=Pushcut",
)
...
def send_to_pushcut(text, km_number):
    """Envoie le feedback Gemini à l'automation Pushcut (POST JSON).

    Retourne (ok: bool, info: str). Le texte est passé dans le champ `input`
    et `text` du corps, exploitables côté automation Pushcut.
    """
    if not PUSHCUT_URL:
        return (False, "no_url")
    try:
