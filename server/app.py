...
Variables d'environnement :
  ...
  PUSHCUT_SECRET      (optionnel)              secret webhook Pushcut
  PUSHCUT_SHORTCUT    (optionnel)              Raccourci déclenché au tap
  PUSHCUT_NOTIFICATION(optionnel)              nom de la notif Pushcut
  PUSHCUT_NOTIF_ID    (optionnel)              id (remplace la notif précédente)

Notification Pushcut :
  À chaque km, UNE SEULE notification est envoyée avec le feedback Gemini
  complet en texte. Un appui dessus (defaultAction) déclenche le Raccourci
  PUSHCUT_SHORTCUT sur l'iPhone, avec le texte en `input`.
...
# n'est pas à jour.
APP_VERSION = "pushcut-single-tap-1"
...
# Pushcut : UNE SEULE notification par km, contenant le feedback Gemini
# complet. Un appui sur la notification (defaultAction) déclenche le Raccourci
# iOS `PUSHCUT_SHORTCUT`, qui reçoit le texte en `input` (ex: pour le lire à
# voix haute, l'afficher, etc.). Fonctionne écran verrouillé ; pas besoin que
# Pushcut ou les Raccourcis soient au premier plan pour RECEVOIR la
# notification (seul le tap déclenche le Raccourci).
...
# Identifiant de notification : si défini, chaque nouveau km REMPLACE la
# notification précédente (au lieu de s'empiler). Laisser vide pour désactiver.
PUSHCUT_NOTIF_ID = os.environ.get("PUSHCUT_NOTIF_ID", "garmini-feedback")

log.info(
    "DEMARRAGE %s | shortcut=%s | notif=%s | id=%s",
    APP_VERSION, PUSHCUT_SHORTCUT, PUSHCUT_NOTIFICATION,
    PUSHCUT_NOTIF_ID or "(aucun)",
)
...
def send_to_pushcut(text, km_number):
    """Envoie UNE SEULE notification Pushcut contenant le feedback Gemini complet.

    Un appui sur la notification (defaultAction) déclenche le Raccourci
    `PUSHCUT_SHORTCUT` sur l'iPhone, avec le texte complet passé en `input`.
    ...
    payload = {
        ...
        "input": text,
        "defaultAction": {
            "shortcut": PUSHCUT_SHORTCUT,
            "input": text,
        },
    }
    if PUSHCUT_NOTIF_ID:
        # Même id -> cette notif remplace la précédente, pas d'empilement.
